...
| Five simultaneous service/Hokim columns | Keep the fixed left-to-right order Ҳокимга оид, Сув, Электр, Газ, Чиқинди and independent vertical Lane scrolling within a fluid full-width layout. |
...
| Semantic Lane differentiation | Keep distinct per-Lane accent/surface pairs (Coral Red, Civic Blue, Violet Elektr, Flame Orange Gaz, Emerald Green Chiqindi) and consistent outline icons, always paired with text meaning. |
...
|---|---|
| Brand & Product Identity | Use the approved Soft Sky/Azure Blue emblem (`#0284C7`) featuring a vector silhouette of a person projecting voice waves (`🗣️`) paired with the **Mahalla Ovozi** wordmark. |
...
| Visible five Lanes only | Add `Йўналишlar: N/5` multi-select for any one-or-more fixed Lanes; never allow zero Lanes or custom categories. |
...
| Colorful functional icon cues | Use distinct semantic outline colors: Coral Red for Hokim (`#EF4444`), Civic Blue for Water (`#2563EB`), Violet for Electricity (`#7C3AED`), Flame Orange for Gas (`#EA580C`), and Emerald Green for Waste (`#059669`). Pair meanings with text and honor reduced motion. |
...
|---|---|
| Red logo and red brand accent | Replaced with the approved Soft Sky/Azure Blue emblem (`#0284C7`) and speaking voice silhouette (`🗣️`). |
...
| Paper-plane action inside quote previews | Telegram navigation belongs to each complete evidence item in detail, labelled clearly as `Telegramda ochish`, and is best effort. |
