...
| Language & evidence | Product-facing MVP UI text and AI-generated Topic summaries use Uzbek Cyrillic. Technical identifiers/model names/error codes keep their original representation. Telegram evidence remains verbatim in its original language and script; do not translate or transliterate it for storage, search, or display. |
...
| Jobs/idempotency | Jobs are retryable. Logical business effects require explicit idempotency/uniqueness keys; infrastructure delivery claims never replace application idempotency. Ordering-sensitive processing follows AD-3 deterministic source-order serialization. |
...
| Tests | Use Vitest 3.x for backend/frontend test execution and Playwright 1.x for critical browser journeys; favor integration/E2E behavior over low-value implementation-detail tests. All tests interacting with PostgreSQL or pg-boss queues must run strictly against an isolated test database (`mahalla_ovozi_test`), never the shared development database. |
