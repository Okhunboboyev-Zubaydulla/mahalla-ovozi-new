...
- Application roles are limited to Product Owner and Hokim. There is no public registration; passwords require 15–128 Unicode code points, must not be silently truncated, and account/session revocation must take effect deterministically. Authenticated browser sessions use a 12-hour genuine-user inactivity boundary and a 24-hour absolute lifetime.
