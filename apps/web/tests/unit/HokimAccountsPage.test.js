...
describe('HokimAccountsPage Component Tests', () => {
    ...
    it('renders NO_ACCOUNT state with creation CTA when district has no Hokim account (AC 1)', async () => {
        ...
        renderWithProviders(_jsx(HokimAccountsPage, { districtId: "dist_test_1" }));
    ...
    it('renders prompt when no district is selected', async () => {
        renderWithProviders(_jsx(HokimAccountsPage, { districtId: undefined }));
        expect(screen.getByText(/Ҳоким аккаунтини бошқариш учун аввал юқоридаги менюдан туманни танланг/i)).toBeTruthy();
    });
});
