...
describe('District State & Switching Engine', () => {
    ...
    const wrapper = ({ children }) => (_jsx(QueryClientProvider, { client: queryClient, children: _jsx(DistrictProvider, { children: children }) }));
    ...
    it('registers and unregisters dirty state with auto-cleanup on unmount (P4-D)', () => {
        const { result, rerender, unmount } = renderHook(({ isDirty }) => {
    ...
    it('intercepts switch transition when dirty and displays UnsavedChangesModal (P5-G)', async () => {
        const TestComponent = () => {
            ...
            return (_jsxs("div", { children: [_jsx("button", { onClick: () => switchDistrict('dist_new'), children: "Switch" }), _jsx(UnsavedChangesModal, {})] }));
        };
        render(_jsx(QueryClientProvider, { client: queryClient, children: _jsx(DistrictProvider, { children: _jsx(TestComponent, {}) }) }));
