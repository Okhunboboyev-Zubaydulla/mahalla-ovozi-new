...
export function createRequireHokim(db: DbClient) {
  return async function requireHokim(req: FastifyRequest, reply: FastifyReply): Promise<void> {
    ...
    if (validation.account.role !== 'DISTRICT_HOKIM' || !validation.account.districtId) {
